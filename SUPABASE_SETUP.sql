...
-- ============================================================
-- الإشعارات: إنشاء تلقائي للأحداث المهمة
-- ============================================================
-- يستخدم SECURITY DEFINER لأن إدراج الإشعار يتم من خلال أحداث النظام،
-- وليس مباشرة من المستخدم العادي.
...
begin
  if tg_op = 'INSERT' then
    perform public.create_system_notification(
      ...
      'طلب تواصل جديد',
      'لديك طلب تواصل جديد يحتاج إلى مراجعتك.',
      'contact_request',
      '/contact-requests',
      new.id
    );
    ...
    if new.status = 'approved' then
      perform public.create_system_notification(
        new.sender_user_id,
        'تم قبول طلب التواصل',
        'تم قبول طلب التواصل الخاص بك ويمكنك متابعة المحادثة.',
        'contact_request',
        '/messages',
        new.id
      );
      ...
      perform public.create_system_notification(
        new.sender_user_id,
        'تم رفض طلب التواصل',
        'تم رفض طلب التواصل الخاص بك بعد مراجعة المشرف.',
        'contact_request',
        '/contact-requests',
        new.id
      );
...
create trigger contact_requests_notifications_insert
...
create trigger contact_requests_notifications_update
...
create trigger profile_views_notifications
after insert on public.profile_views
for each row execute function public.notify_profile_view_event();
